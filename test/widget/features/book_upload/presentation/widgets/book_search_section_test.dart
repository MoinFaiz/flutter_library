...
void main() {
  ...
  group('BookSearchSection Tests', () {
    ...
    testWidgets('disables search button when searching', (WidgetTester tester) async {
      ...
      whenListen(
        mockBookUploadBloc,
        Stream<BookUploadState>.value(searchingState),
        initialState: searchingState,
      );

      // Act
      await tester.pumpWidget(createTestWidget());

      // Assert - Find the ElevatedButton by type (it contains CircularProgressIndicator when searching)
      final searchButtonFinder = find.byType(ElevatedButton);
      expect(searchButtonFinder, findsOneWidget);
      final searchButton = tester.widget<ElevatedButton>(searchButtonFinder);
      expect(searchButton.onPressed, isNull);
      // Verify it shows loading indicator
    ...
    testWidgets('displays search results when available', (WidgetTester tester) async {
      ...
      whenListen(
        mockBookUploadBloc,
        Stream<BookUploadState>.value(stateWithResults),
        initialState: stateWithResults,
      );
    ...
    testWidgets('shows no results when search returns empty', (WidgetTester tester) async {
      // Arrange
      ...
      whenListen(
        mockBookUploadBloc,
        Stream<BookUploadState>.value(stateWithNoResults),
        initialState: stateWithNoResults,
      );

      // Act
      await tester.pumpWidget(createTestWidget());

      // Assert - When no results, the results list should not be shown
